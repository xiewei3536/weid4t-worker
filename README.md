...
| 分頁 | 內容 |
|---|---|
| 📊 **總覽** | 總裝置 / 線上 / 今日上線 / 已授權 / 未授權 / 未用碼;**直播源伺服器健康**(讀訂閱來源同源的 `status.json`,由 VPS 看門狗每 5 分鐘實測頻道與切片後輸出:正常 / 異常(含自動處置) / 停止回報);授權、OTA、公告、跑馬燈狀態橫幅;**正在觀看**(哪台盒子在看哪個頻道);**App 版本分布**(幾台還是舊版);快速操作 |
